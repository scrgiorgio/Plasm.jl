...
"""
	normalcoords(V::Points, EV; flag=true::Bool)::Points
...
"""
function normalcoords(V::Points, EV; flag=true)
	...
	if flag
		...
		hpc = STRUCT(
		    T(1,2)(0,ty) , S(1,2)(1,-1) , S(1,2)(umax-umin, vmax-vmin) ,
			 S(1,2)(1/(xmax-xmin),1/(ymax-ymin)) , T(1,2)(-xmin,-ymin) ,
			 MKPOL(V, EV))
	else
		# T = Lar.t(0, ymax-ymin) * Lar.s(1,-1)
		hpc = STRUCT( S(1,2)(1,-1), MKPOL(V, EV))
	end
	return hpc
end
...
"""
function SVG(filename::String; flag=true)
	outlines = Vector{Float64}[]
